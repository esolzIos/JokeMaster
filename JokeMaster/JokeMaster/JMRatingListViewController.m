...
@interface JMRatingListViewController ()
{
    // int listcount;
...
@implementation JMRatingListViewController
...
-(void)loadData
{
    ...
    if([self networkAvailable])
    {
        ...
        //http://ec2-13-58-196-4.us-east-2.compute.amazonaws.com/jokemaster/index.php/useraction/rateduserlisting?videoid=&page=1&limit=10&mode=1
        
        url=[NSString stringWithFormat:@"%@%@Useraction/rateduserlisting?videoid=%@&page=%d&limit=15&mode=%@",GLOBALAPI,INDEX,_videoId,page,[[NSUserDefaults standardUserDefaults] objectForKey:@"langmode"]];
...
-(void) tableView:(UITableView *)tableView willDisplayCell:(JMFavouriteCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.RankLabel setText:[NSString stringWithFormat:@"RATED %@/5",[videoDict objectForKey:@"rating"]]];
    ...
    [cell.CountryName setText:[NSString stringWithFormat:@"%@",[videoDict objectForKey:@"country"]]];
        ...
        cell.RatingView.value = [[videoDict objectForKey:@"rating"] floatValue];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSDictionary *videoDict=[videoArr objectAtIndex:indexPath.row];
            ...
            VC.ProfileUserId=[videoDict valueForKey:@"user_id"];
